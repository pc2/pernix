...
auto unpack_epi32_fallback_inner(const T* __restrict__ input, const uint8_t bit_offset, const std::size_t elements)
    -> std::vector<int32_t> {
...
auto unpack_epi32_fallback(const uint8_t* __restrict__ input, const std::size_t elements) -> std::vector<int32_t> {
    if constexpr (BIT_WIDTH >= 1 && BIT_WIDTH <= 8) {
        return internal::unpack_epi32_fallback_inner<uint8_t, BIT_WIDTH, SIGN_VALUES>(reinterpret_cast<const uint8_t*>(input), 0, elements);
    } else if constexpr (BIT_WIDTH >= 9 && BIT_WIDTH <= 16) {
        return internal::unpack_epi32_fallback_inner<uint16_t, BIT_WIDTH, SIGN_VALUES>(reinterpret_cast<const uint16_t*>(input), 0,
                                                                                       elements);
    } else {
        return internal::unpack_epi32_fallback_inner<uint32_t, BIT_WIDTH, SIGN_VALUES>(reinterpret_cast<const uint32_t*>(input), 0,
                                                                                       elements);
    }
}

auto unpack_epi32_fallback(uint8_t bit_width, const uint8_t* __restrict__ input, std::size_t elements) -> std::vector<int32_t>;
}  // namespace libcompression::bitpacking
