...
namespace detail {
inline constexpr usize neon_vector_width = 128;
...
template <typename ShiftType, u8 BIT_WIDTH, usize ELEMENTS>
constexpr std::array<ShiftType, ELEMENTS> make_shift_right() {
    ...
    for (usize entry = 0; entry < ELEMENTS; ++entry) {
        const usize bit_start  = entry * BIT_WIDTH;
        const usize bit_offset = bit_start % 8u;
...
template <typename ShiftType, u8 BIT_WIDTH, usize LANE_BITS, usize ELEMENTS>
...
template <u8 BIT_WIDTH, u8 START_BIT_OFFSET, usize ELEMENTS>
constexpr std::array<u8, ELEMENTS> make_contiguous_permute_32() {
    ...
    for (usize entry = 0; entry < ELEMENTS / 4; ++entry) {
        const usize bit_start  = START_BIT_OFFSET + entry * BIT_WIDTH;
        const usize bit_end    = bit_start + BIT_WIDTH - 1;
        const usize first_byte = bit_start / 8;
        const usize last_byte  = bit_end / 8;
        const usize base       = entry * 4;

        for (usize byte = first_byte; byte <= last_byte; ++byte) {
...
private:
    static constexpr usize PERMUTE_ELEMENTS = VECTOR_WIDTH / 8;
    static constexpr usize SHIFT_ELEMENTS   = VECTOR_WIDTH / 8;
...
private:
    static constexpr usize PERMUTE_ELEMENTS = VECTOR_WIDTH / 8;
    static constexpr usize SHIFT_ELEMENTS   = VECTOR_WIDTH / 16;
...
private:
    static constexpr usize PERMUTE_ELEMENTS = VECTOR_WIDTH / 8;
    static constexpr usize SHIFT_ELEMENTS   = VECTOR_WIDTH / 32;
