#ifndef PERNIX_UNPACKING_AVX512_H
#define PERNIX_UNPACKING_AVX512_H
...
__m128i mm_unpack_epi32_avx512vbmi(const uint8_t* __restrict__ input) {
    if constexpr (BIT_WIDTH > 0 && BIT_WIDTH <= 8) {
        ...
        const __m128i shuffled = _mm_permutexvar_epi8(internal::unpack_tables_avx512_8<BIT_WIDTH, __m128i>::get_shuffle(), source);

        __m128i shifted = _mm_sllv_epi16(shuffled, internal::unpack_tables_avx512_8<BIT_WIDTH, __m128i>::get_shift());
        ...
        const __m128i shuffled = _mm_permutexvar_epi8(internal::unpack_tables_avx512_24<BIT_WIDTH, __m128i>::get_shuffle(), source);

        constexpr uint16_t shift = 32 - BIT_WIDTH;
        __m128i shifted          = _mm_sllv_epi32(shuffled, internal::unpack_tables_avx512_24<BIT_WIDTH, __m128i>::get_shift());
        if constexpr (SIGN_VALUES) {
...
__m256i mm256_unpack_epi32_avx512vbmi(const uint8_t* __restrict__ input) {
    if constexpr (BIT_WIDTH > 0 && BIT_WIDTH <= 8) {
        ...
        const __m256i shuffled = _mm256_permutexvar_epi8(internal::unpack_tables_avx512_24<BIT_WIDTH, __m256i>::get_shuffle(), source);

        constexpr uint16_t shift = 32 - BIT_WIDTH;
        __m256i shifted          = _mm256_sllv_epi32(shuffled, internal::unpack_tables_avx512_24<BIT_WIDTH, __m256i>::get_shift());
        if constexpr (SIGN_VALUES) {
...
auto mm512_unpack_aligned_epi32_avx512vbmi(const uint8_t* __restrict__ input) -> __m512i {
...
auto mm512_unpack_epi32_avx512vbmi(const uint8_t* __restrict__ input) -> __m512i {
    if constexpr (BIT_WIDTH > 0 && BIT_WIDTH <= 8) {
        ...
        const __m256i permuted = _mm256_permutexvar_epi8(internal::unpack_tables_avx512_8<BIT_WIDTH, __m256i>::get_shuffle(), source);

        __m256i shifted = _mm256_sllv_epi16(permuted, internal::unpack_tables_avx512_8<BIT_WIDTH, __m256i>::get_shift());
        ...
        const __m512i permuted = _mm512_permutexvar_epi8(internal::unpack_tables_avx512_24<BIT_WIDTH, __m512i>::get_shuffle(), source);

        __m512i shifted = _mm512_sllv_epi32(permuted, internal::unpack_tables_avx512_24<BIT_WIDTH, __m512i>::get_shift());
...
__m128i mm_unpack_epi32_avx512vbmi(const uint8_t* __restrict__ input) {
    if constexpr (BIT_WIDTH == 8 || BIT_WIDTH == 16) {
        ...
        return internal::mm_unpack_epi32_avx512vbmi<BIT_WIDTH, SIGN_VALUES>(input);
    }
}
...
__m256i mm256_unpack_epi32_avx512vbmi(const uint8_t* __restrict__ input) {
    if constexpr (BIT_WIDTH == 8 || BIT_WIDTH == 16) {
        ...
        return internal::mm256_unpack_epi32_avx512vbmi<BIT_WIDTH, SIGN_VALUES>(input);
    }
}
...
auto mm512_unpack_epi32_avx512vbmi(const uint8_t* __restrict__ input) -> __m512i {
    if constexpr (BIT_WIDTH == 8 || BIT_WIDTH == 16) {
        ...
        return internal::mm512_unpack_epi32_avx512vbmi<BIT_WIDTH, SIGN_VALUES>(input);
    }
}
...
#endif  // PERNIX_UNPACKING_AVX512_H
