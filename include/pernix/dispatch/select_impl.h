...
#define PERNIX_BLOCK_SIZE_COMPRESS_BLOCKS_SWITCH_64(BS) \
    ...
    inline Kernel<KernelBlockF32Func> select_fallback_stdpar_compress_block_f32(const u8, const u32) {
#if defined(PERNIX_BUILD_FALLBACK_STDPAR)
        return {"fallback_stdpar", &compress_block_fallback_stdpar<1, 64, f32>};
#else
        return {"unsupported_implementation", nullptr};
#endif
    }

    inline Kernel<KernelBlocksF32Func> select_fallback_stdpar_compress_blocks_f32(const u8, const u32) {
#if defined(PERNIX_BUILD_FALLBACK_STDPAR)
        return {"fallback_stdpar", &compress_blocks_fallback_stdpar<1, 64, f32>};
#else
        return {"unsupported_implementation", nullptr};
#endif
    }

    inline Kernel<KernelBlockF64Func> select_fallback_stdpar_compress_block_f64(const u8, const u32) {
#if defined(PERNIX_BUILD_FALLBACK_STDPAR)
        return {"fallback_stdpar", &compress_block_fallback_stdpar<1, 64, f64>};
#else
        return {"unsupported_implementation", nullptr};
#endif
    }

    inline Kernel<KernelBlocksF64Func> select_fallback_stdpar_compress_blocks_f64(const u8, const u32) {
#if defined(PERNIX_BUILD_FALLBACK_STDPAR)
        return {"fallback_stdpar", &compress_blocks_fallback_stdpar<1, 64, f64>};
#else
        return {"unsupported_implementation", nullptr};
#endif
    }
...
#define PERNIX_BLOCK_SIZE_DECOMPRESS_SWITCH_BLOCKS_64(BS) \
    ...
    inline Kernel<KernelBlockF32Func> select_fallback_stdpar_decompress_block_f32(const u8, const u32,
                                                                                   const bool sign_values) {
        static_cast<void>(sign_values);
#if defined(PERNIX_BUILD_FALLBACK_STDPAR)
        if (sign_values) {
            return {"fallback_stdpar", &decompress_block_fallback_stdpar<1, true, 64, f32>};
        }
        return {"fallback_stdpar", &decompress_block_fallback_stdpar<1, false, 64, f32>};
#else
        return {"unsupported_implementation", nullptr};
#endif
    }

    inline Kernel<KernelBlocksF32Func> select_fallback_stdpar_decompress_blocks_f32(const u8, const u32,
                                                                                     const bool sign_values) {
        static_cast<void>(sign_values);
#if defined(PERNIX_BUILD_FALLBACK_STDPAR)
        if (sign_values) {
            return {"fallback_stdpar", &decompress_blocks_fallback_stdpar<1, true, 64, f32>};
        }
        return {"fallback_stdpar", &decompress_blocks_fallback_stdpar<1, false, 64, f32>};
#else
        return {"unsupported_implementation", nullptr};
#endif
    }

    inline Kernel<KernelBlockF64Func> select_fallback_stdpar_decompress_block_f64(const u8, const u32,
                                                                                   const bool sign_values) {
        static_cast<void>(sign_values);
#if defined(PERNIX_BUILD_FALLBACK_STDPAR)
        if (sign_values) {
            return {"fallback_stdpar", &decompress_block_fallback_stdpar<1, true, 64, f64>};
        }
        return {"fallback_stdpar", &decompress_block_fallback_stdpar<1, false, 64, f64>};
#else
        return {"unsupported_implementation", nullptr};
#endif
    }

    inline Kernel<KernelBlocksF64Func> select_fallback_stdpar_decompress_blocks_f64(const u8, const u32,
                                                                                     const bool sign_values) {
        static_cast<void>(sign_values);
#if defined(PERNIX_BUILD_FALLBACK_STDPAR)
        if (sign_values) {
            return {"fallback_stdpar", &decompress_blocks_fallback_stdpar<1, true, 64, f64>};
        }
        return {"fallback_stdpar", &decompress_blocks_fallback_stdpar<1, false, 64, f64>};
#else
        return {"unsupported_implementation", nullptr};
#endif
    }
