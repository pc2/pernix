...
__always_inline stdpar_buffer<u32, GROUP_SIZE>
quantize_epi32_group_fallback_stdpar(const std::span<const ScaleType> input, const ScaleType scale) {
...
__always_inline void pack_epi32_group_fallback_stdpar(const stdpar_buffer<u32, GROUP_SIZE>& input,
...
int compress_block_fallback_stdpar(const void* input_ptr, ScaleType scale, void* output_ptr) {
    ...
    if constexpr (remainder != 0) {
        ...
        internal::pack_epi32_group_fallback_stdpar<BIT_WIDTH, tail_group_size>(
            quantized, std::span<u8>(output + tail_output_offset, tail_bytes));
    }
...
int compress_blocks_fallback_stdpar(const void* input_ptr, ScaleType scale, void* output_ptr, u32 blocks) {
    ...
    std::for_each_n(std::execution::par_unseq, first, blocks, [&](const u32 block) {
        ...
        static_cast<void>(compress_block_fallback_stdpar<BIT_WIDTH, BLOCK_SIZE, ScaleType>(block_input,
                                                                                           scale,
                                                                                           block_output));
    });
