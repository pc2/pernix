...
template <u8 LANES>
    requires(LANES <= 8)
__always_inline __m256i mm256_tail_mask_epi32() {
    return _mm256_setr_epi32(LANES > 0 ? -1 : 0, LANES > 1 ? -1 : 0, LANES > 2 ? -1 : 0, LANES > 3 ? -1 : 0, LANES > 4 ? -1 : 0,
                             LANES > 5 ? -1 : 0, LANES > 6 ? -1 : 0, LANES > 7 ? -1 : 0);
}

template <u8 LANES>
    requires(LANES <= 4)
__always_inline __m256i mm256_tail_mask_epi64() {
    return _mm256_setr_epi64x(LANES > 0 ? -1 : 0, LANES > 1 ? -1 : 0, LANES > 2 ? -1 : 0, LANES > 3 ? -1 : 0);
}
 ...
 */
__always_inline static __m128i mm_blend_epi8(const __m128i X, const __m128i Y, const i8 M) {
 ...
 */
__always_inline static __m256i mm256_blend_epi8(const __m256i X, const __m256i Y, const i8 M) {
...
__always_inline auto mm_pack_epi32_avx2_1to3(__m128i input) -> __m128i {
    ...
    const u32 packed = (lanes[0] & bitmask) | ((lanes[1] & bitmask) << BIT_WIDTH) | ((lanes[2] & bitmask) << (2 * BIT_WIDTH)) |
                       ((lanes[3] & bitmask) << (3 * BIT_WIDTH));

    return _mm_cvtsi32_si128(static_cast<i32>(packed));
}
...
__always_inline auto mm256_pack_epi32_avx2_9to16(__m256i input) -> __m256i {
    using tables          = pack_tables_avx2_16<BIT_WIDTH, __m128i>;
    constexpr u16 bitmask = (1U << BIT_WIDTH) - 1U;
...
__always_inline auto mm256_pack_epi32_avx2_5to7(__m256i input) -> __m256i {
    const __m256i zero   = _mm256_setzero_si256();
    const __m256i masked = _mm256_and_si256(input, _mm256_set1_epi32((1U << BIT_WIDTH) - 1U));
...
template <u8 N, usize... I>
    requires(N <= 32)
__always_inline __m256i dword_store_mask(std::index_sequence<I...>) {
...
template <u8 N>
    requires(N <= 32)
__always_inline void mm256_storeu_partial_epi32(void* output, const __m256i values) {
    if constexpr (N == 0) {
        return;
    }

    if constexpr (N == 4) {
        _mm_storeu_si32(output, _mm256_castsi256_si128(values));
        return;
    }

    if constexpr (N == 8) {
        _mm_storeu_si64(output, _mm256_castsi256_si128(values));
        return;
    }

    if constexpr (N == 16) {
        _mm_storeu_si128(static_cast<__m128i*>(output), _mm256_castsi256_si128(values));
        return;
    }

    if constexpr (N == 32) {
        _mm256_storeu_si256(static_cast<__m256i*>(output), values);
        return;
    }

    const __m256i dmask = dword_store_mask<N>(std::make_index_sequence<8>{});
    _mm256_maskstore_epi32(static_cast<int*>(output), dmask, values);
}
}  // namespace internal
...
int mm256_compress_block_avx2(const void* __restrict__ input_ptr, const f32 scale, void* __restrict__ output_ptr) {
...
#pragma GCC unroll 8
    for (u32 iter = 0; iter < iterations_8; iter++) {
        ...
        internal::mm256_storeu_partial_epi32<BIT_WIDTH>(output, packed);
...
template <u8 BIT_WIDTH, u32 BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
int mm256_compress_block_avx2(const void* __restrict__ input_ptr, const f64 scale, void* __restrict__ output_ptr) {
...
template <u8 BIT_WIDTH, u32 BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
int mm256_compress_blocks_avx2(const void* __restrict__ input_ptr, const f32 scale, void* __restrict__ output_ptr, const u32 blocks) {
    const auto* input = static_cast<const float*>(input_ptr);
    auto* output      = static_cast<u8*>(output_ptr);
...
template <u8 BIT_WIDTH, u32 BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
int mm256_compress_blocks_avx2(const void* __restrict__ input_ptr, const f64 scale, void* __restrict__ output_ptr, const u32 blocks) {
    const auto* input = static_cast<const double*>(input_ptr);
    auto* output      = static_cast<u8*>(output_ptr);
