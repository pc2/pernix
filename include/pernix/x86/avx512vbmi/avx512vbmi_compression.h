...
namespace pernix {
    namespace internal {
        ...
        static __always_inline __m512i mm512_clamp_signed_epi32(__m512i input) {
            ...
            return _mm512_min_epi32(_mm512_max_epi32(input, _mm512_set1_epi32(min_value)),
                                    _mm512_set1_epi32(max_value));
        }
        ...
        static __always_inline __m512i mm512_clamp_signed_epi64(__m512i input) {
            ...
            return _mm512_min_epi64(_mm512_max_epi64(input, _mm512_set1_epi64(min_value)),
                                    _mm512_set1_epi64(max_value));
        }
 ...
 */
        ...
        static __always_inline __m512i make_m512i_from_4x128(const __m128i a, const __m128i b, const __m128i c,
                                                             const __m128i d) {
        ...
        static __always_inline __m512i make_m512i_from_8x64(const __m128i a, const __m128i b, const __m128i c,
                                                            const __m128i d, const __m128i e,
        ...
        static __always_inline __m256i make_m256i_from_4x64(const __m128i a, const __m128i b, const __m128i c,
                                                            const __m128i d) {
            const __m128i ab = _mm_unpacklo_epi64(a, b);
            const __m128i cd = _mm_unpacklo_epi64(c, d);
...
__always_inline int mm512_compress_block_avx512vbmi_1to8(const f32 * __restrict__ input, const f32 scale,
            ...
            constexpr u32 remaining_elements =
                    elements_per_block - iterations_64 * 64 - iterations_32 * 32 - iterations_16 * 16;
...
#pragma GCC unroll 8
                for (u32 iter = 0; iter < iterations_64; ++iter) {
                    ...
                    const __m512i quantized1 = mm512_clamp_signed_epi32<BIT_WIDTH>(
                        mm512_quantize_ps_epi32(source1, scale_v));
                    const __m512i quantized2 = mm512_clamp_signed_epi32<BIT_WIDTH>(
                        mm512_quantize_ps_epi32(source2, scale_v));
                    const __m512i quantized3 = mm512_clamp_signed_epi32<BIT_WIDTH>(
                        mm512_quantize_ps_epi32(source3, scale_v));
                    const __m512i quantized4 = mm512_clamp_signed_epi32<BIT_WIDTH>(
                        mm512_quantize_ps_epi32(source4, scale_v));
                    ...
                    const __m512i packed =
                            m512::mm512_pack_epi8_avx512vbmi_1to8 < BIT_WIDTH > (make_m512i_from_4x128(
            ...
            if constexpr (iterations_32 > 0) {
                ...
                const __m256i packed = m256::mm256_pack_epi8_avx512vbmi_1to8 < BIT_WIDTH > (make_m256i_from_2x128(
...
__always_inline int mm512_compress_block_avx512vbmi_9to16(const f32 * __restrict__ input, const f32 scale,
                                                          u8 * __restrict__ output) {
            constexpr u32 elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;
            ...
            constexpr u32 remaining_elements =
                    elements_per_block - iterations_32 * 32 - iterations_16 * 16 - iterations_8 * 8;
...
#pragma GCC unroll 4
                for (u32 iter = 0; iter < iterations_32; ++iter) {
                    ...
                    const __m512i packed = m512::mm512_pack_epi16_avx512vbmi_9to16 < BIT_WIDTH > (make_m512i_from_2x256(
                                               converted1, converted2));
            ...
            if constexpr (iterations_8 > 0) {
                ...
                const __m256i quantized = mm256_clamp_signed_epi32_avx512<BIT_WIDTH>(
                    mm256_quantize_ps_epi32(source, scale_v256));
...
__always_inline int mm512_compress_block_avx512vbmi_17to24(const f32 * __restrict__ input, const f32 scale,
                                                           u8 * __restrict__ output) {
            constexpr u32 elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;
...
#pragma GCC unroll 2
                for (u32 i = 0; i < iterations_16; ++i) {
                    const __m512 source = _mm512_loadu_ps(input);
                    const __m512i packed_input = mm512_clamp_signed_epi32<BIT_WIDTH>(
                        mm512_quantize_ps_epi32(source, scale_v));
            ...
            if constexpr (iterations_8 > 0) {
                const __m256 source = _mm256_loadu_ps(input);
                const __m256i packed_input = mm256_clamp_signed_epi32_avx512<BIT_WIDTH>(
                    mm256_quantize_ps_epi32(source, scale_v256));
        ...
        template<u8 BIT_WIDTH, u32 BLOCK_SIZE>
            requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 8) && (BLOCK_SIZE % 32 == 0)
__always_inline int mm512_compress_block_avx512vbmi_1to8(const f64 * __restrict__ input, const f64 scale,
                                                         u8 * __restrict__ output) {
            constexpr u32 elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;
...
#pragma GCC unroll 8
                for (u32 iter = 0; iter < iterations_64; ++iter) {
                    ...
                    const __m512i quantized1 = mm512_clamp_signed_epi64<BIT_WIDTH>(
                        mm512_quantize_pd_epi64(source1, scale_v));
                    const __m512i quantized2 = mm512_clamp_signed_epi64<BIT_WIDTH>(
                        mm512_quantize_pd_epi64(source2, scale_v));
                    const __m512i quantized3 = mm512_clamp_signed_epi64<BIT_WIDTH>(
                        mm512_quantize_pd_epi64(source3, scale_v));
                    const __m512i quantized4 = mm512_clamp_signed_epi64<BIT_WIDTH>(
                        mm512_quantize_pd_epi64(source4, scale_v));
                    const __m512i quantized5 = mm512_clamp_signed_epi64<BIT_WIDTH>(
                        mm512_quantize_pd_epi64(source5, scale_v));
                    const __m512i quantized6 = mm512_clamp_signed_epi64<BIT_WIDTH>(
                        mm512_quantize_pd_epi64(source6, scale_v));
                    const __m512i quantized7 = mm512_clamp_signed_epi64<BIT_WIDTH>(
                        mm512_quantize_pd_epi64(source7, scale_v));
                    const __m512i quantized8 = mm512_clamp_signed_epi64<BIT_WIDTH>(
                        mm512_quantize_pd_epi64(source8, scale_v));
                    ...
                    const __m512i packed = m512::mm512_pack_epi8_avx512vbmi_1to8 < BIT_WIDTH > (
                                               make_m512i_from_8x64(converted1, converted2, converted3, converted4,
                                                                    converted5, converted6, converted7, converted8));
            ...
            if constexpr (iterations_32 > 0) {
                ...
                const __m256i packed =
                        m256::mm256_pack_epi8_avx512vbmi_1to8 < BIT_WIDTH > (make_m256i_from_4x64(
                            converted1, converted2, converted3, converted4));
            ...
            if constexpr (iterations_16 > 0) {
                ...
                const __m128i packed = m128::mm_pack_epi8_avx512vbmi_1to8 < BIT_WIDTH > (_mm_unpacklo_epi64(
                                           converted1, converted2));
            ...
            if constexpr (remaining_elements > 0) {
                ...
                const __m512d source2 = source2_elements > 0
                                            ? mm512_loadu_elements_pd(source2_elements, input + 8)
                                            : _mm512_setzero_pd();
                const __m512i quantized1 = mm512_clamp_signed_epi64<BIT_WIDTH>(
                    mm512_quantize_pd_epi64(source1, scale_v));
                const __m512i quantized2 = mm512_clamp_signed_epi64<BIT_WIDTH>(
        ...
        template<u8 BIT_WIDTH, u32 BLOCK_SIZE>
            requires(BIT_WIDTH >= 9 && BIT_WIDTH <= 16) && (BLOCK_SIZE % 32 == 0)
__always_inline int mm512_compress_block_avx512vbmi_9to16(const f64 * __restrict__ input, const f64 scale,
                                                          u8 * __restrict__ output) {
            constexpr u32 elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;
...
#pragma GCC unroll 4
                for (u32 iter = 0; iter < iterations_32; ++iter) {
                    ...
                    const __m512i packed =
                            m512::mm512_pack_epi16_avx512vbmi_9to16 < BIT_WIDTH > (make_m512i_from_4x128(
                                converted1, converted2, converted3, converted4));
            ...
            if constexpr (iterations_16 > 0) {
                ...
                const __m256i packed = m256::mm256_pack_epi16_avx512vbmi_9to16 < BIT_WIDTH > (make_m256i_from_2x128(
                                           converted1, converted2));
        ...
        template<u8 BIT_WIDTH, u32 BLOCK_SIZE>
            requires(BIT_WIDTH >= 17 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
__always_inline int mm512_compress_block_avx512vbmi_17to24(const f64 * __restrict__ input, const f64 scale,
                                                           u8 * __restrict__ output) {
            constexpr u32 elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;
...
#pragma GCC unroll 2
                for (u32 i = 0; i < iterations_16; ++i) {
                    ...
                    const __m256i quantized1 = mm256_clamp_signed_epi32_avx512<BIT_WIDTH>(
                        mm512_quantize_pd_epi32(source1, scale_v));
                    const __m256i quantized2 = mm256_clamp_signed_epi32_avx512<BIT_WIDTH>(
                        mm512_quantize_pd_epi32(source2, scale_v));

                    const __m512i packed = m512::mm512_pack_epi32_avx512vbmi_17to24 < BIT_WIDTH > (
                                               make_m512i_from_2x256(quantized1, quantized2));
                    mm512_storeu_elements_epi16(output, BIT_WIDTH, packed);
            ...
            if constexpr (iterations_8 > 0) {
                const __m512d source = _mm512_loadu_pd(input);
                const __m256i quantized = mm256_clamp_signed_epi32_avx512<BIT_WIDTH>(
                    mm512_quantize_pd_epi32(source, scale_v));
 ...
 */
    ...
    int mm512_compress_block_avx512vbmi(const void * __restrict__ input_ptr, const f32 scale,
 ...
 */
    template<u8 BIT_WIDTH, u32 BLOCK_SIZE>
        requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
    int mm512_compress_block_avx512vbmi(const void * __restrict__ input_ptr, const f64 scale,
                                        void * __restrict__ output_ptr) {
 ...
 */
    template<u8 BIT_WIDTH, u32 BLOCK_SIZE>
        requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
    int mm512_compress_blocks_avx512vbmi(const void * __restrict__ input_ptr, const f32 scale,
                                         void * __restrict__ output_ptr,
                                         const u32 blocks) {
        const auto *input = static_cast<const float *>(input_ptr);
        auto *output = static_cast<u8 *>(output_ptr);
 ...
 */
    template<u8 BIT_WIDTH, u32 BLOCK_SIZE>
        requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
    int mm512_compress_blocks_avx512vbmi(const void * __restrict__ input_ptr, const f64 scale,
                                         void * __restrict__ output_ptr,
                                         const u32 blocks) {
        const auto *input = static_cast<const double *>(input_ptr);
        auto *output = static_cast<u8 *>(output_ptr);
