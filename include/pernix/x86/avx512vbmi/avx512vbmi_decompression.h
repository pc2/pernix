...
__always_inline int mm512_decompress_block_avx512vbmi_1to8(const u8* __restrict__ input, const f32 scale,
    ...
    const u32 remaining_elements = elements_per_block - iterations_64 * 64 - iterations_32 * 32 -
                                   iterations_16 * 16;
...
#pragma GCC unroll 8
        for (u32 i = 0; i < iterations_64; ++i) {
            ...
            const __m512i unpacked = m512::mm512_unpack_epi8_avx512vbmi_1to8<BIT_WIDTH, SIGN_VALUES
                >
                (source);
    ...
    if constexpr (iterations_32 > 0) {
        ...
        const __m256i unpacked = m256::mm256_unpack_epi8_avx512vbmi_1to8<BIT_WIDTH, SIGN_VALUES
            >
            (source);
    ...
    if constexpr (iterations_16 > 0) {
        ...
        const __m128i unpacked = m128::mm_unpack_epi8_avx512vbmi_1to8<BIT_WIDTH, SIGN_VALUES
            >
            (source);
...
template <u8 BIT_WIDTH, bool SIGN_VALUES = true, u32 BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 8) && (BLOCK_SIZE % 32 == 0)
__always_inline int mm512_decompress_block_avx512vbmi_1to8(const u8* __restrict__ input, const f64 scale,
...
__always_inline int mm512_decompress_block_avx512vbmi_9to16(const u8* __restrict__ input, const f32 scale,
                                                            f32* __restrict__ output) {
    constexpr u32 elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;
...
#pragma GCC unroll 4
        for (u32 i = 0; i < iterations_32; ++i) {
            ...
            const __m512i unpacked = m512::mm512_unpack_epi16_avx512vbmi_9to16<BIT_WIDTH, SIGN_VALUES
                >
                (source);
    ...
    if constexpr (iterations_16 > 0) {
        ...
        const __m256i unpacked = m256::mm256_unpack_epi16_avx512vbmi_9to16<BIT_WIDTH, SIGN_VALUES
            >
            (source);
    ...
    if constexpr (iterations_8 > 0) {
        ...
        const __m128i unpacked = m128::mm_unpack_epi16_avx512vbmi_9to16<BIT_WIDTH, SIGN_VALUES
            >
            (source);
...
template <u8 BIT_WIDTH, bool SIGN_VALUES = true, u32 BLOCK_SIZE>
    requires(BIT_WIDTH >= 9 && BIT_WIDTH <= 16) && (BLOCK_SIZE % 32 == 0)
__always_inline int mm512_decompress_block_avx512vbmi_9to16(const u8* __restrict__ input, const f64 scale,
                                                            f64* __restrict__ output) {
    constexpr u32 elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;
...
__always_inline int mm512_decompress_block_avx512vbmi_17to24(const u8* __restrict__ input, const f32 scale,
                                                             f32* __restrict__ output) {
    constexpr u32 elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;
...
#pragma GCC unroll 2
        for (u32 i = 0; i < iterations_16; ++i) {
            ...
            const __m512i unpacked = m512::mm512_unpack_epi32_avx512vbmi_17to24<BIT_WIDTH, SIGN_VALUES
                >
                (source);
    ...
    if constexpr (iterations_8 > 0) {
        ...
        const __m256i unpacked = m256::mm256_unpack_epi32_avx512vbmi_17to24<BIT_WIDTH, SIGN_VALUES
            >
            (source);
...
template <u8 BIT_WIDTH, bool SIGN_VALUES = true, u32 BLOCK_SIZE>
    requires(BIT_WIDTH >= 17 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
__always_inline int mm512_decompress_block_avx512vbmi_17to24(const u8* __restrict__ input, const f64 scale,
                                                             f64* __restrict__ output) {
    constexpr u32 elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;
...
__always_inline int mm512_decompress_block_avx512vbmi(const void* __restrict__ input_ptr, const f32 scale,
    ...
    if constexpr (BIT_WIDTH >= 1 && BIT_WIDTH <= 8) {
        return internal::mm512_decompress_block_avx512vbmi_1to8<BIT_WIDTH, SIGN_VALUES, BLOCK_SIZE>(
            input, scale, output);
    } else if constexpr (BIT_WIDTH >= 9 && BIT_WIDTH <= 16) {
        return internal::mm512_decompress_block_avx512vbmi_9to16<BIT_WIDTH, SIGN_VALUES, BLOCK_SIZE>(
            input, scale, output);
    } else if constexpr (BIT_WIDTH >= 17 && BIT_WIDTH <= 24) {
        return internal::mm512_decompress_block_avx512vbmi_17to24<BIT_WIDTH, SIGN_VALUES, BLOCK_SIZE>(
            input, scale, output);
    }
    return 0;
}
...
template <u8 BIT_WIDTH, bool SIGN_VALUES = true, u32 BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
__always_inline int mm512_decompress_block_avx512vbmi(const void* __restrict__ input_ptr, const f64 scale,
                                                      void* __restrict__ output_ptr) {
    const auto* input = static_cast<const u8*>(input_ptr);
...
template <u8 BIT_WIDTH, bool SIGN_VALUES = true, u32 BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
int mm512_decompress_blocks_avx512vbmi(const void* __restrict__ input_ptr, const f32 scale,
                                       void* __restrict__ output_ptr,
                                       const u32 blocks) {
    const auto* input = static_cast<const u8*>(input_ptr);
    auto* output      = static_cast<f32*>(output_ptr);
...
template <u8 BIT_WIDTH, bool SIGN_VALUES = true, u32 BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
int mm512_decompress_blocks_avx512vbmi(const void* __restrict__ input_ptr, const f64 scale,
                                       void* __restrict__ output_ptr,
                                       const u32 blocks) {
    const auto* input = static_cast<const u8*>(input_ptr);
    auto* output      = static_cast<f64*>(output_ptr);
