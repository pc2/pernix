...
#define PERNIX_COMPRESS_BLOCK_CASE(N) \
    case N:                           \
        return compress_block_fallback<N>(input, scale, output);

#define PERNIX_COMPRESS_BLOCKS_CASE(N) \
    case N:                            \
        return compress_blocks_fallback<N>(input, scale, output, blocks);
...
int compress_block_fallback_f64(const uint8_t bit_width, const double_t* __restrict__ input, const double_t scale,
...
int compress_blocks_fallback_f64(const uint8_t bit_width, const double_t* __restrict__ input, const double_t scale,
