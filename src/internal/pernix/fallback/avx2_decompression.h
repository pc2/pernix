...
__always_inline auto unpack_epi32_fallback_inner(const uint8_t* __restrict__ input, const uint8_t bit_offset,
                                                 const std::size_t elements)
    -> std::vector<int32_t> {
    constexpr uint32_t bits_in_type = sizeof(T) * 8;
    constexpr uint32_t bitmask      = BIT_WIDTH == bits_in_type
...
int decompress_block_fallback(const void* __restrict__ input_ptr, const float_t scale,
                              void* __restrict__ output_ptr) {
    const auto* input = static_cast<const uint8_t*>(input_ptr);
    auto* output      = static_cast<float_t*>(output_ptr);

    constexpr uint32_t elements_per_block = (BLOCK_SIZE * 8) / BIT_WIDTH;

    const std::vector<int32_t> block_values = internal::unpack_epi32_fallback<BIT_WIDTH, SIGN_VALUES>(
...
template <uint8_t BIT_WIDTH, bool SIGN_VALUES = true, uint32_t BLOCK_SIZE>
    requires(BIT_WIDTH > 0 && BIT_WIDTH <= 24)
int decompress_block_fallback(const void* __restrict__ input_ptr, const double_t scale,
                              void* __restrict__ output_ptr) {
    const auto* input = static_cast<const uint8_t*>(input_ptr);
    auto* output      = static_cast<double_t*>(output_ptr);
...
template <uint8_t BIT_WIDTH, bool SIGN_VALUES = true, uint32_t BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24)
int decompress_blocks_fallback(const void* __restrict__ input_ptr, const float_t scale,
...
template <uint8_t BIT_WIDTH, bool SIGN_VALUES = true, uint32_t BLOCK_SIZE>
    requires(BIT_WIDTH > 0 && BIT_WIDTH <= 24)
int decompress_blocks_fallback(const void* __restrict__ input_ptr, const double_t scale,
                               void* __restrict__ output_ptr, const uint32_t blocks) {
    const auto* input = static_cast<const uint8_t*>(input_ptr);
    auto* output      = static_cast<double_t*>(output_ptr);
