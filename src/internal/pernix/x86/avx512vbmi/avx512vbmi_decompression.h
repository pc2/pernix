...
__always_inline int mm512_decompress_block_avx512vbmi(const void* __restrict__ input_ptr, const float_t scale,
...
template <uint8_t BIT_WIDTH, bool SIGN_VALUES = true, uint32_t BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
__always_inline int mm512_decompress_block_avx512vbmi(const void* __restrict__ input_ptr, const double_t scale,
                                                      void* __restrict__ output_ptr) {
    const auto* input = static_cast<const uint8_t*>(input_ptr);
...
template <uint8_t BIT_WIDTH, bool SIGN_VALUES = true, uint32_t BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
int mm512_decompress_blocks_avx512vbmi(const void* __restrict__ input_ptr, const float_t scale, void* __restrict__ output_ptr,
...
template <uint8_t BIT_WIDTH, bool SIGN_VALUES = true, uint32_t BLOCK_SIZE>
    requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
int mm512_decompress_blocks_avx512vbmi(const void* __restrict__ input_ptr, const double_t scale, void* __restrict__ output_ptr,
                                       const uint32_t blocks) {
    const auto* input = static_cast<const uint8_t*>(input_ptr);
    auto* output      = static_cast<double_t*>(output_ptr);
