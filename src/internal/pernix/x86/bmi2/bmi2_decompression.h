        ...
        __m256i mm256_unpack_epi32_bmi2(const u8 * __restrict__ input) {
            ...
            if constexpr (BIT_WIDTH >= 1 && BIT_WIDTH <= 8) {
                ...
                if constexpr ((BIT_WIDTH % 2) == 0) {
                    ...
                    temp_values[2] = (raw_values[0] >> second_group_shift) | (
                                         raw_values[1] << (64 - second_group_shift));
                ...
                result = _mm256_set_epi64x(static_cast<i64>(values[3]), static_cast<i64>(values[2]),
                                           static_cast<i64>(values[1]),
 ...
 */
    ...
    int mm256_decompress_block_bmi2(const void * __restrict__ input_ptr, const f32 scale,
 ...
 */
    template<u8 BIT_WIDTH, bool SIGN_VALUES = true, u32 BLOCK_SIZE>
        requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
    int mm256_decompress_block_bmi2(const void * __restrict__ input_ptr, const f64 scale,
                                    void * __restrict__ output_ptr) {
        const auto *input = static_cast<const u8 *>(input_ptr);
 ...
 */
    template<u8 BIT_WIDTH, bool SIGN_VALUES = true, u32 BLOCK_SIZE>
        requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
    int mm256_decompress_blocks_bmi2(const void * __restrict__ input_ptr, const f32 scale,
                                     void * __restrict__ output_ptr,
                                     const u32 blocks) {
        const auto *input = static_cast<const u8 *>(input_ptr);
        auto *output = static_cast<f32 *>(output_ptr);
 ...
 */
    template<u8 BIT_WIDTH, bool SIGN_VALUES = true, u32 BLOCK_SIZE>
        requires(BIT_WIDTH >= 1 && BIT_WIDTH <= 24) && (BLOCK_SIZE % 32 == 0)
    int mm256_decompress_blocks_bmi2(const void * __restrict__ input_ptr, const f64 scale,
                                     void * __restrict__ output_ptr,
                                     const u32 blocks) {
        const auto *input = static_cast<const u8 *>(input_ptr);
