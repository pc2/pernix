...
int main(void) {
    ...
    pernix_status status = pernix_compress_block_f32(PERNIX_BACKEND_FALLBACK, bit_width, block_size, input,
                                                     1.0f / scale, compressed);
    if (status != PERNIX_STATUS_OK) {
        return 1;
    }

    status = pernix_decompress_block_f32(PERNIX_BACKEND_FALLBACK, bit_width, block_size, compressed, scale, restored,
                                         true);
    if (status != PERNIX_STATUS_OK) {
        return 2;
    }
