...
std::vector<FloatT> make_dataset(const Pattern pattern, const u8 bit_width, const u32 blocks = kBlocks) {
    ...
    for (usize i = 0; i < values.size(); ++i) {
        const FloatT unit = values.size() > 1
                                ? static_cast<FloatT>((static_cast<double>(i) / static_cast<double>(values.size() - 1)) * 2.0 - 1.0)
                                : FloatT{0};
        switch (pattern) {
            case Pattern::Zero:
                values[i] = FloatT{0};
            ...
            case Pattern::QuantizationBoundary: {
                ...
                if (one_bit) {
                    ...
                    const FloatT nudge = (i % 2U == 0U) ? step * static_cast<FloatT>(0.49) : step * static_cast<FloatT>(-0.49);
...
template <typename FloatT>
pernix_status compress_blocks(pernix_backend backend, const u8 bit_width, const std::vector<FloatT>& input, const FloatT inverse_scale,
                              std::vector<u8>& compressed, const u32 blocks) {
    if constexpr (std::is_same_v<FloatT, float>) {
        return pernix_compress_blocks_f32(backend, bit_width, kBlockSize, input.data(), inverse_scale, compressed.data(), blocks);
    } else {
        return pernix_compress_blocks_f64(backend, bit_width, kBlockSize, input.data(), inverse_scale, compressed.data(), blocks);
    }
}

template <typename FloatT>
pernix_status decompress_blocks(pernix_backend backend, const u8 bit_width, const std::vector<u8>& compressed, const FloatT scale,
                                std::vector<FloatT>& output, const u32 blocks, const bool sign_values = true) {
    if constexpr (std::is_same_v<FloatT, float>) {
        return pernix_decompress_blocks_f32(backend, bit_width, kBlockSize, compressed.data(), scale, output.data(), blocks, sign_values);
    } else {
        return pernix_decompress_blocks_f64(backend, bit_width, kBlockSize, compressed.data(), scale, output.data(), blocks, sign_values);
    }
}

template <typename FloatT>
void expect_fallback_round_trip(const u8 bit_width, const PatternCase& pattern_case) {
    SCOPED_TRACE(::testing::Message() << "bit_width=" << static_cast<unsigned>(bit_width) << ", pattern=" << pattern_case.name);
    ...
    ASSERT_EQ(compress_blocks(PERNIX_BACKEND_FALLBACK, bit_width, input, FloatT{1} / scale, compressed, kBlocks), PERNIX_STATUS_OK);
    ASSERT_EQ(decompress_blocks(PERNIX_BACKEND_FALLBACK, bit_width, compressed, scale, restored, kBlocks), PERNIX_STATUS_OK);
...
void expect_backend_matches_fallback(const pernix_backend backend, const u8 bit_width) {
    SCOPED_TRACE(::testing::Message() << "backend=" << static_cast<int>(backend) << ", bit_width=" << static_cast<unsigned>(bit_width));
    ...
    ASSERT_EQ(compress_blocks(PERNIX_BACKEND_FALLBACK, bit_width, input, FloatT{1} / scale, fallback_compressed, 2), PERNIX_STATUS_OK);
    ...
    if (compress_status != PERNIX_STATUS_OK) {
        EXPECT_TRUE(compress_status == PERNIX_STATUS_UNSUPPORTED_IMPLEMENTATION || compress_status == PERNIX_STATUS_UNSUPPORTED_BACKEND ||
    ...
    ASSERT_EQ(decompress_blocks(PERNIX_BACKEND_FALLBACK, bit_width, fallback_compressed, scale, fallback_restored, 2), PERNIX_STATUS_OK);
    ASSERT_EQ(decompress_blocks(PERNIX_BACKEND_FALLBACK, bit_width, backend_compressed, scale, backend_restored, 2), PERNIX_STATUS_OK);

    const FloatT tolerance =
        (abs_value(scale) * static_cast<FloatT>(1.1)) + (std::numeric_limits<FloatT>::epsilon() * static_cast<FloatT>(64));
    for (usize i = 0; i < input.size(); ++i) {
        ASSERT_NEAR(backend_restored[i], fallback_restored[i], tolerance) << "element=" << i << ", scale=" << scale;
    }

    std::vector<FloatT> backend_decompressed(input.size(), FloatT{});
    const auto decompress_status = decompress_blocks(backend, bit_width, fallback_compressed, scale, backend_decompressed, 2);
    if (decompress_status != PERNIX_STATUS_OK) {
        EXPECT_TRUE(decompress_status == PERNIX_STATUS_UNSUPPORTED_IMPLEMENTATION ||
                    decompress_status == PERNIX_STATUS_UNSUPPORTED_BACKEND || decompress_status == PERNIX_STATUS_UNSUPPORTED_BIT_WIDTH);
        return;
    }
    for (usize i = 0; i < input.size(); ++i) {
        ASSERT_NEAR(backend_decompressed[i], fallback_restored[i], tolerance) << "element=" << i << ", scale=" << scale;
    }
}
}  // namespace
...
TEST(DeterministicCorrectnessTest, F32AndF64HaveConsistentFallbackSemantics) {
    ...
    ASSERT_EQ(compress_blocks(PERNIX_BACKEND_FALLBACK, bit_width, input_f32, 1.0f, compressed_f32, 2), PERNIX_STATUS_OK);
    ASSERT_EQ(compress_blocks(PERNIX_BACKEND_FALLBACK, bit_width, input_f64, 1.0, compressed_f64, 2), PERNIX_STATUS_OK);
    ...
    ASSERT_EQ(decompress_blocks(PERNIX_BACKEND_FALLBACK, bit_width, compressed_f32, 1.0f, restored_f32, 2), PERNIX_STATUS_OK);
    ASSERT_EQ(decompress_blocks(PERNIX_BACKEND_FALLBACK, bit_width, compressed_f64, 1.0, restored_f64, 2), PERNIX_STATUS_OK);
...
TEST(DeterministicCorrectnessTest, CppSpanWrappersMatchCApiOutputAndStatus) {
    ...
    EXPECT_EQ(pernix_compress_block_f32(PERNIX_BACKEND_FALLBACK, bit_width, kBlockSize, input.data(), 1.0f / scale, c_compressed.data()),
              PERNIX_STATUS_OK);
    EXPECT_EQ(pernix::compress_block(pernix::Backend::Fallback, bit_width, kBlockSize, std::span<const float>(input), 1.0f / scale,
                                     std::span<u8>(cpp_compressed)),
              PERNIX_STATUS_OK);
    ...
    EXPECT_EQ(
        pernix_decompress_block_f32(PERNIX_BACKEND_FALLBACK, bit_width, kBlockSize, c_compressed.data(), scale, c_restored.data(), true),
        PERNIX_STATUS_OK);
    EXPECT_EQ(pernix::decompress_block(pernix::Backend::Fallback, bit_width, kBlockSize, std::span<const u8>(cpp_compressed), scale,
                                       std::span<float>(cpp_restored)),
              PERNIX_STATUS_OK);
    EXPECT_EQ(cpp_restored, c_restored);

    EXPECT_EQ(pernix::compress_block(pernix::Backend::Fallback, 0, kBlockSize, std::span<const float>(input), 1.0f / scale,
                                     std::span<u8>(cpp_compressed)),
              pernix_compress_block_f32(PERNIX_BACKEND_FALLBACK, 0, kBlockSize, input.data(), 1.0f / scale, c_compressed.data()));
}
...
TEST(DeterministicCorrectnessTest, SpanValidationRejectsUndersizedMultiBlockBuffers) {
    ...
    EXPECT_EQ(pernix::compress_blocks(pernix::Backend::Fallback, bit_width, kBlockSize,
                                      std::span<const float>(input).first(input.size() - 1U), 1.0f, std::span<u8>(compressed), 2),
              PERNIX_STATUS_INVALID_ARGUMENT);
    EXPECT_EQ(pernix::compress_blocks(pernix::Backend::Fallback, bit_width, kBlockSize, std::span<const float>(input), 1.0f,
                                      std::span<u8>(compressed).first(compressed.size() - 1U), 2),
              PERNIX_STATUS_INVALID_ARGUMENT);
    EXPECT_EQ(pernix::decompress_blocks(pernix::Backend::Fallback, bit_width, kBlockSize,
                                        std::span<const u8>(compressed).first(compressed.size() - 1U), 1.0f, std::span<float>(restored), 2),
              PERNIX_STATUS_INVALID_ARGUMENT);
    EXPECT_EQ(pernix::decompress_blocks(pernix::Backend::Fallback, bit_width, kBlockSize, std::span<const u8>(compressed), 1.0f,
