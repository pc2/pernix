...
int main() {
    ...
    if (pernix::compress_block(pernix::Backend::Fallback, bit_width, block_size, input, 8.0f, compressed) !=
        PERNIX_STATUS_OK) {
        return 1;
    }
    if (pernix::decompress_block(pernix::Backend::Fallback, bit_width, block_size, compressed, 0.125f, restored) !=
        PERNIX_STATUS_OK) {
        return 2;
    }
